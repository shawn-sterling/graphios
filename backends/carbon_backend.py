...
def convert_pickle(metrics):
	"""
		Converts the metric object list into a pickle formatted message and returns it
	"""
	pickle_list = []
	for m in metrics:
		path = "%s.%s.%s.%s"%(m.GRAPHITEPREFIX,m.HOSTNAME,m.GRAPHITEPOSTFIX,m.LABEL)
		value = m.LABEL
		timestamp = m.TIMET
...
def send(metrics):
	...
	try:
		...
		log.warning("Can't connect to carbon: %s:%s %s" % (carbon_server,carbon_port, ex))
